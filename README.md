...
指导思想：
- 可维护性：修改功能，需要改动的地方越少越好，可维护性就越好
- 可复用性：代码可以被以后重复使用，写出自己的类库
- 可扩展性：添加功能无需修改原来的代码
- 灵活性：代码接口可以灵活调用
...
## 2. 开闭原则 Open-Close Principe
对扩展开放，对修改关闭：尽量不修改原来代码的情况下，进行扩展
抽象化：`多态`是开闭原则的关键

## 3. 里氏替换原则
所有使用父类的地方，必须能够透明的使用子类对象（子类能够替换父类）
...
常见的设计模式有：
单例模式、工厂模式、代理模式、观察者模式（MQ消息监听）、责任链模式（Filter）、策略模式、 
